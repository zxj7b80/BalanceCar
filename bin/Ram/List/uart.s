///////////////////////////////////////////////////////////////////////////////
//                                                                            /
// IAR ANSI C/C++ Compiler V6.10.1.52143/W32 for ARM    05/Jan/2012  22:42:53 /
// Copyright 1999-2010 IAR Systems AB.                                        /
//                                                                            /
//    Cpu mode     =  thumb                                                   /
//    Endian       =  little                                                  /
//    Source file  =  F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\Sources\C\Comp /
//                    onent_C\uart.c                                          /
//    Command line =  F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\Sources\C\Comp /
//                    onent_C\uart.c -D IAR -D TWR_K60N512 -lCN               /
//                    F:\Kinetis\IARK60X256\06_LQ_ADC_UART\bin\Ram\List\ -lB  /
//                    F:\Kinetis\IARK60X256\06_LQ_ADC_UART\bin\Ram\List\ -o   /
//                    F:\Kinetis\IARK60X256\06_LQ_ADC_UART\bin\Ram\Obj\       /
//                    --no_cse --no_unroll --no_inline --no_code_motion       /
//                    --no_tbaa --no_clustering --no_scheduling --debug       /
//                    --endian=little --cpu=Cortex-M4 -e --fpu=None           /
//                    --dlib_config "C:\Program Files\IAR Systems\Embedded    /
//                    Workbench 6.0\arm\INC\c\DLib_Config_Normal.h" -I        /
//                    F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\Sources\H\ -I  /
//                    F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\Sources\H\Comp /
//                    onent_H\ -I F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\So /
//                    urces\H\Frame_H\ -Ol --use_c++_inline                   /
//    List file    =  F:\Kinetis\IARK60X256\06_LQ_ADC_UART\bin\Ram\List\uart. /
//                    s                                                       /
//                                                                            /
//                                                                            /
///////////////////////////////////////////////////////////////////////////////
...
// F:\Kinetis\IARK60X256\06_LQ_ADC_UART\src\Sources\C\Component_C\uart.c
//    1 //-------------------------------------------------------------------------*
//    2 // 文件名: uart.c                                                          *
//    3 // 说  明: uart构件源文件                                                  *
//    4 //-------------------------------------------------------------------------*
...
//  224 //-------------------------------------------------------------------------
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  225 void uart_sendstring (UART_MemMapPtr uartch ,uint8* buff)
//  226 {
uart_sendstring:
        PUSH     {R4-R6,LR}
        MOVS     R4,R0
...
//  227     int i;
//  228 
//  229 	for(i=0;buff[i] != 0;i++)
        MOVS     R6,#+0
        B.N      ??uart_sendstring_0
//  230     {
//  231 		uart_send1(uartch,buff[i]);
??uart_sendstring_1:
        LDRB     R1,[R6, R5]
        MOVS     R0,R4
        BL       uart_send1
...
//  233 }
        POP      {R4-R6,PC}       ;; return
//  234 //-------------------------------------------------------------------------
//  235 //函数名称：hw_get_byte_array_from_number                                               
//  236 //功能概要：将数组number转换成ASCII码的形式放到数组中                                                
//  237 //参数说明：number: 要转换的数字                                                   
//  238 //          length: 长度        
//  239 //           array: 转换完的结果数组
//  240 //函数返回： 无                                                               
//  241 //-------------------------------------------------------------------------
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  242 uint8 get_byte_array_from_number(uint8 array [], int * length, int number)
//  243 {
...
//  266     *length = i;
        STR      R3,[R1, #+0]
//  267     //将解析的ASCII码数据存放在array中
//  268     for(j = 0; j < i; j++)
        MOVS     R4,#+0
        B.N      ??get_byte_array_from_number_4
//  269     {
//  270         array[j] = pBuff[i - j - 1] + '0';
...
//  276 
//  277 //-------------------------------------------------------------------------
//  278 //函数名称：hw_uart_sendnumber                                               
//  279 //功能概要：将数字转换成字符串发送出去  如123转换成"123"发送出去                                                
//  280 //参数说明：uartch: 串口号                                                   
//  281 //           buff: 要发送的值                                                 
//  282 //函数返回： 无                                                               
//  283 //-------------------------------------------------------------------------
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  284 uint8 uart_sendnumber(UART_MemMapPtr uartch , int number)
//  285 {
uart_sendnumber:
        PUSH     {R0-R4,LR}
        MOVS     R4,R0
...
//  287     int len = 0;
        MOVS     R0,#+0
        STR      R0,[SP, #+0]
//  288     if(get_byte_array_from_number(array, &len, number) == 0)
        MOVS     R2,R1
        ADD      R1,SP,#+0
        ADD      R0,SP,#+4
...
//  289     {
//  290     	uart_sendN(uartch, (uint8*)array, len);
        LDR      R2,[SP, #+0]
